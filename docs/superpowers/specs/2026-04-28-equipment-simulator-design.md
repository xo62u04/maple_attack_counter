...
### 6-3 隊伍 BUFF 也出現在攻擊力計算機（Tab 1）

Tab 1 的左欄底部（存檔區上方）新增「隊伍 BUFF」勾選區塊，邏輯與裝備模擬器相同：
- 勾選後即時影響 Tab 1 的攻速/ATK/爆擊率等數值
- BUFF 勾選狀態與數值和 Tab 1 存檔一起儲存
- BUFF 清單來自同一份 `data.json partyBuffs`，共用資料

### 6-4 裝備模擬器「從攻擊力計算機匯入」

裝備模擬器頂部「裸裝屬性」區塊旁有「📥 從攻擊力計算機匯入」按鈕：
- 將 Tab 1 目前填入的 STR / DEX / INT / LUK / ATK / ATK% / 技能% / 熟練度 / 爆擊設定 / 防禦設定帶入裝備模擬器的裸裝欄位與技能設定欄位
- 所有裝備槽保持空白（使用者再逐一填入）
- 方便「懶人流程」：先用 Tab 1 算好總數，想進一步拆解時直接導入模擬器繼續細填
